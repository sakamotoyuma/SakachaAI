...
embedding_encoding = "cl100k_base"          # エンベディングのパラメータの設定
max_tokens = 1500                           # -------------------------------
...
tokenizer = tiktoken.get_encoding(embedding_encoding)   # 「text」のトークン数を計算し、新しい列「n_tokens」に格納
df['n_tokens'] = df.text.apply(lambda x: len(tokenizer.encode(x)))
def aplit_into_many (text, max_tokens = 500):
    ...
    for sentence, token in zip(sentences, n_tokens):
        ...
        # 最大のトークン数を超える場合は、チャンクをチャンクのリストに追加し、
        # チャンクとトークン数をリセット
        ...
        # それ以外の場合は文をチャンクに追加し、トークン数を合計に追加
